import { getparentLineNode, getRangeAtLineStart, getRangeAtLineEnd } from './DOMHelper';
...
const withCtrlCodes = [
    ...
    65, // a   ctrl-a  goto line start
    69, // e   ctrl-e  goto line end
];

const lineOpKey = {65: 'move-line-start', 69: 'move-line-end'};
...
let getRangeByKey = (oldRange, {keyCode, ctrlKey, shiftKey, metaKey}) => {
    let ran = document.createRange();
    let cmd;
    // 将带有ctrl的移动键 转化为普通的箭头键
    if(ctrlKey){
        let mapedKey = ctrlMoveMap[keyCode];
        if(mapedKey){
            cmd = getCMDByKey(oldRange, mapedKey, shiftKey)
        }
        else{
            cmd = lineOpKey[keyCode];
        }
    }

    let _range = getRangeByCMD(oldRange, cmd);
    if(_range instanceof window.Range){
        return _range;
    }
    let {startNode, endNode, startOffset, endOffset} = _range;

    ran.setStart(startNode, startOffset);
...
let getRangeByCMD = (oldRange, cmd) => {
    ...
    switch(cmd) {
        case 'go-to-start':
            ...
            startOffset = endOffset = oldRange.startOffset;
            break;
        case 'go-to-end':
            startNode = endNode = oldRange.endContainer;
            startOffset = endOffset = oldRange.endOffset;
            break;
        case 'move-start':
            startNode = endNode = oldRange.startContainer;
            startOffset = endOffset = oldRange.startOffset - 1;
            break;
        case 'move-end':
            startNode = endNode = oldRange.endContainer;
            startOffset = endOffset = oldRange.endOffset + 1;
            break;
        case 'move-line-start':
            return getRangeAtLineStart(getparentLineNode(oldRange.startContainer), oldRange)
            break;
        case 'move-line-end':
            return getRangeAtLineEnd(getparentLineNode(oldRange.endContainer), oldRange)
            break;
    }
