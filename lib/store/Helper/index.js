...
let replaceContent = (contentData, startId, endId, startOffset, endOffset, replacer) => {
    if(startId === endId){
        ...
        // TODO: 优化遍历
        contentData.some(function(val, key){
            if(key === startId){
...
let Delete = (Store, direction) => {
    ...
    if(range.collapsed){
        if(direction === 'back'){
            ...
            // backspace 在行首再一次触发时
            if(replaceStart < 0){
                let prevLineId = findPrevId(content, startId);
                let prevLineText = content.get(prevLineId);
            ...
            // delete 在行尾再一次触发时
            let thisLineText = content.get(startId);
            if(relplaceEnd > thisLineText.length){
                let nextLineId = findNextId(content, startId);
                relplaceEnd = 0;
                endId = nextLineId;
            }
        }
